...
- **Admin actions (server-wide)**: popup exposes two destructive operations under an Admin disclosure:
  ...
  - **Reset server state** sends `admin_reset_state`; server replaces in-memory state with the empty v3 shape, writes to disk, and broadcasts the empty `state_update` so other clients see the wipe immediately. Any authenticated client can invoke these — auth is the only gate.
