...
- **First-device-wins initial sync with confirmation**: when a device connects and the server is empty, that device pushes its state with `replace:true` and becomes the authoritative seed. When a subsequent device connects and finds the server non-empty, sync stays PAUSED (`initialSyncDone=false`, no patches flow) until the user opens the popup and confirms a destructive replace. Confirm → `applyState({addOnly:false})` deletes local tabs/workspaces not in server; Cancel → disconnects. This makes the "different initial states" case predictable: the first device defines the truth, others opt in.
