...
- **`state_update` broadcasts apply additively only**: when a peer pushes `full_state` and the server broadcasts the merged state back, receivers do NOT use `addOnly:false` (which would remove their local-only tabs that aren't in the broadcast). Otherwise a tab the user just closed on one device gets re-created when a peer's slightly-stale full_state arrives. Tab removals propagate exclusively through `remove_tab` patch ops.
- **Initial-sync prompt picks a direction**: when a device connects for the first time, sync stays PAUSED until the user explicitly chooses. Popup shows server state (workspaces/folders/essentials/tabs counts) AND local state, with two destructive buttons: "Replace local (use server)" → `applyState({addOnly:false})` wipes local-only records; "Push local (overwrite server)" → `sendFullState(local, {replace:true})` wipes server state. Cancel → disconnect. The prompt is suppressed only when both sides are empty (nothing to negotiate). This explicit direction-picking prevents the race where two devices reconnecting near-simultaneously to an empty server let whichever's WebSocket handshake completed first silently overwrite the other.
- **Replace-local removal pass queries live browser state**: the destructive "Replace local" path in `_applyState({addOnly:false})` used to iterate `tabMonitor.state.tabs/folders/workspaces` for the removal pass. If capture was incomplete (e.g. transient runtime API issue), tabs that were in `gBrowser` but missed by `tabMonitor.state` survived the replace, leaving "old tabs still hanging around" after the user opted to fully replace. Removal now queries `browser.zenInternals.listLiveUrls()` and `getRuntimeState()` directly, so the live `gBrowser` and `gZenWorkspaces` snapshot drives deletions instead of the cached monitor state.
- **Admin actions (server-wide)**: popup exposes two destructive operations under an Admin disclosure:
  - **Disable sync on all** sends `admin_disable_all` to the server; server forwards `force_disable` to every connected client, each of which turns its `syncEnabled` storage flag off and disconnects.
  - **Reset server state** sends `admin_reset_state`; server wipes its state AND broadcasts `force_disable` to every connected client (including the sender). The implicit force-disable matters: without it the admin client's next periodic capture would re-emit patches and silently rebuild the state from scratch, making the reset look like it did nothing. After reset, the user re-enables sync on the first device manually, which then re-seeds the server (`!initialSyncDone + totalRemoteTabs==0 → sendFullState(replace:true)`). Any authenticated client can invoke these — auth is the only gate.
...
- **Workspace reorder via Zen internal**: `reorderWorkspaces({orderedUuids})` wraps `gZenWorkspaces.reorderWorkspace(uuid, position)` from `src/zen/spaces/ZenSpaceManager.mjs` upstream. `_applyState` translates remote `workspaces[].position` to ordered local UUIDs and calls it. Best-effort: silently no-ops on Zen versions that lack the method.
- **Folder reorder via DOM**: `reorderFolders({orderedFolderIds, parentId})` walks the folder DOM and `insertBefore`s in reverse order, same pattern as `reorderTabsInPlace`. There is no upstream Zen API for folder reordering; the DOM approach works at runtime but may not survive a full Zen restart (Zen rebuilds folder order from sessionstore). Acceptable for live propagation.
- **Title/icon coalescing in diff**: `_computePatch` drops `update_tab` ops whose ONLY meaningful change is `title` or `icon` AND the title looks like a loading-state placeholder (URL string, length < 3, or title === url). The final stable title arrives within seconds and emits then. Eliminates the bulk of mid-page-load update-flood patches.
